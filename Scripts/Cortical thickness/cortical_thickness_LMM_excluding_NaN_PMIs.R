...
M1 = lmer(Cortical_thickness ~ Iron + Age_at_death + Sex_0_male_1_female + PMI + (1|Brain) + (1|Lobe), data=no_NaN_data_csv, na.action = na.exclude, REML = FALSE)
#error: some predictor variables are on very different scales: consider rescaling
summary(M1)

#Null model before testing assumptions
M0 = lmer(Cortical_thickness ~ Age_at_death + Sex_0_male_1_female + PMI + (1|Brain) + (1|Lobe), data=no_NaN_data_csv, na.action = na.exclude, REML = FALSE)
summary(M0)
...
(ggplot(pred.mm) + 
    ...
    geom_point(data = no_NaN_data_csv, size = 2,                      # adding the raw data
               aes(x = Iron, y = Cortical_thickness, colour = factor(Lobe))) + 
    xlim(0, 27000) +
    ylim(1.5, 4) +
    labs(x = "Iron deposits", y = "Cortical thickness (mm)") + 
    theme_classic()+
