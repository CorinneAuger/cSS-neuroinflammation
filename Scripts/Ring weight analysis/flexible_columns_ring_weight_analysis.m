...
% Ring weight analysis for each number of columns (1-4). 
...
for number_of_columns = 1:4
    ...
    directory.save_by_section = sprintf('/Volumes/Corinne hard drive/cSS project/Saved data/One-pixel ring weight analysis/%s (1-tailed)/%d column/By section', inflammatory_marker, number_of_columns);
    directory.save_overall_matrix = sprintf('/Volumes/Corinne hard drive/cSS project/Saved data/One-pixel ring weight analysis/%s (1-tailed)/%d column/Composite', inflammatory_marker, number_of_columns);
    
    %% Make matrix of weight combinations
    loop = 1;
    
    if number_of_columns == 1
        percent_weights = [100 0 0 0];
        
    elseif number_of_columns == 2
        percent_weights = NaN(52, 4);
        
        for trial_pixel_weight = 0:100
            for trial_first_ring_weight = 0:100
                for trial_second_ring_weight = 0
                    for trial_third_ring_weight = 0
                        if trial_pixel_weight >= trial_first_ring_weight
    ...
    elseif number_of_columns == 3
        percent_weights = NaN(885, 4);
        
        for trial_pixel_weight = 0:100
            for trial_first_ring_weight = 0:100
                for trial_second_ring_weight = 0:100
                    for trial_third_ring_weight = 0 % edit
                        if trial_pixel_weight >= trial_first_ring_weight && trial_first_ring_weight >= trial_second_ring_weight %edit
                            if  (trial_pixel_weight + trial_first_ring_weight + trial_second_ring_weight + trial_third_ring_weight) == 100
                                loop = loop + 1;
                                percent_weights(loop,:) = [trial_pixel_weight, trial_first_ring_weight, trial_second_ring_weight, trial_third_ring_weight];
                            end
    ...
    elseif number_of_columns == 4
        percent_weights = NaN(8038,4); %one-tailed 4 columns
        %percent_weights = NaN(176852,4); %two-tailed 4 columns
    ...
    [number_of_weight_combos, ~] = size(percent_weights);
    ...
    if number_of_columns == 2 || number_of_columns == 3 || number_of_columns == 4
        weights(1,:) = [];
        number_of_weight_combos = number_of_weight_combos - 1;
    end
    
    clearvars -except weights inflammatory_marker directory number_of_weight_combos number_of_columns
    
    %% Pre-allocate matrices for all blocks
    if number_of_columns == 1
        best_weights_all_blocks = NaN(1,4,104);
    else
        ...
        for block = [1, 4, 5, 7]
            ...
            if isfile(variables_file)
                ...
                if slope > 0
                    ...
                    difference_means = NaN((number_of_weight_combos),1);
                    ...
                    for n = 1:number_of_weight_combos
                        progress = waitbar(n/number_of_weight_combos);
                    ...
                    if number_of_columns == 1
                        for k = 1
                            minimum_indices(k) = find(difference_means_without_minimum(:) == min(difference_means_without_minimum(:)), 1);
                    ...
                    save(sprintf('CAA%d__%d_%s_and_Iron_best_weights_%d_column.mat', brain, block, inflammatory_marker, number_of_columns));
                    ...
                    clearvars -except brain block weights number_of_weight_combos number_of_columns best_weights_all_blocks excluded_based_on_slope minima_all_blocks inflammatory_marker directory inflammation_median
    ...
    % Save variables
    clearvars -except number_of_columns directory weights best_weights_all_blocks section_mean section_median top_weight_for_each_brain top_weight_for_each_section brain_mean brain_median excluded_based_on_slope minima_all_blocks inflammatory_marker
    save(sprintf('%s_and_Iron_1pixel_%d_column_ring_weight_analysis_variables.mat', inflammatory_marker, number_of_columns))
    
    %% Make into csv
    %cd(directory.scripts)
    %ring_weight_csv(inflammatory_marker, number_of_columns);
