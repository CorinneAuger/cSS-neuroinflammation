...
M1 = lmer(Cortical_thickness ~ Iron + Age_at_death + Sex_0_male_1_female + (1|Brain) + (1|Lobe), data=all_data_csv, na.action = na.exclude, REML = FALSE)
#error: some predictor variables are on very different scales: consider rescaling
summary(M1)

#Null model before testing assumptions
M0 = lmer(Cortical_thickness ~ Age_at_death + Sex_0_male_1_female + (1|Brain) + (1|Lobe), data=all_data_csv, na.action = na.exclude, REML = FALSE)
summary(M0)
...
aic(new_null_model, new_full_model)
bic(new_null_model, new_full_model)