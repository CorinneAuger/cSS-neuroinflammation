...
#Define data
all_data_csv = read.csv(file = '/Users/corinneauger/Documents/Aiforia heatmap coregistration/Saved data/CSV files for stats/2-tailed_early_CD68_ring_weight_data.csv')


#Model before testing assumptions
M1 = lmer(Weight ~ Ring + Age_at_death + Sex_0_male_1_female + (1|Brain) + (1|Lobe), data=all_data_csv, na.action = na.exclude, REML = FALSE)
#error: boundary (singular) fit
summary(M1)

#Null model before testing assumptions
M0 = lmer(Weight ~ 1 + Age_at_death + Sex_0_male_1_female + (1|Brain) + (1|Lobe), data=all_data_csv, na.action = na.exclude, REML = FALSE)
summary(M0)

#Sanity check pre-assumption ranova
ranova(M0, M1)
...
ranova(new_null_model, new_full_model)
